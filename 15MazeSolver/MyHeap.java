...
public class MyHeap{
    ...
    private ArrayList<Location> arr=new ArrayList<Location>();
    public MyHeap(){
	arr.add(null);
	maxx=true;
    }
	...
	while(finished==false){
	    ...
	    Location lookingAt=arr.get(lastChildInd);
    ...
    public void swap(int ind1, int ind2, ArrayList<Location> arrr){
	Location ele1=arrr.get(ind1);
        Location ele2=arr.get(ind2);
	arr.set(ind1,ele2);
	arr.set(ind2,ele1);
    }
	...
	if(size()==2){
		...
		if(maxx){
		    Location par=arr.get(parentInd);
		    Location chil1=arr.get(childInd1);
		    if(childInd2<=size()){
			Location chil2=arr.get(childInd2);
			Location larger=new Location();
			int ind=0;
